...
*** Capture key bindings and templates
...
(setq org-capture-templates
  '(("b" "Book" entry (file+datetree "books.org")
         "* %^{Title}  %^{Author}p  %^{Interest}p  %?")
   ("p" "Person" entry (file+headline "people.org" "Capture")
         "* %^{Name} %^{Organization}p %^g %?")))
#+end_src 
*** Smart-quote binding
